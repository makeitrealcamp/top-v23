...
function list(array){
  return cars.map(item => ` This ${item.brand} from ${item.model} costs $${item.price}`)
}

// Create a function bmwSum that returns the sum of all prices of the BMW cars.
function bmwSum(array){
  const bmw = cars.filter(item => item.brand === "BMW")
  const allSum = bmw.reduce((acc,car)=>acc + car.price,0)
  return allSum
}
